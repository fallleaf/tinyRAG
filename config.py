#!/usr/bin/env python3
"""config.py - tinyRAG 配置契约与加载层 (v2.1)"""
import re
...
class ConfidenceConfig(BaseModel):
    type_rules: dict[str, float] = Field(default_factory=lambda: {"code": 1.1, "table": 1.05, "header": 1.0, "text": 0.95, "list": 0.9})
    doc_type_rules: dict[str, float] = Field(default_factory=dict)
    status_rules: dict[str, float] = Field(default_factory=lambda: {"published": 1.2, "completed": 1.1, "active": 0.9, "draft": 0.7, "archived": 0.5})
...
class Settings(BaseModel):
    vaults: list[VaultConfig] = Field(default_factory=lambda: [
...
def _strip_yaml_keys_values(d):
    if isinstance(d, dict):
        return {
            re.sub(r'\s+', '', k): _strip_yaml_keys_values(v) if isinstance(v, (dict, list)) else 
            re.sub(r'^\s+|\s+$', '', v) if isinstance(v, str) else v
            for k, v in d.items()
        }
