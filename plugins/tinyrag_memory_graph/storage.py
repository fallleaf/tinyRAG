...
class GraphStorage:
    ...
    def delete_note(self, note_id: str) -> bool:
        ...
        try:
            # 先删除相关关系
            self.conn.execute(
                "DELETE FROM relations WHERE src_chunk_id IN (SELECT id FROM chunks WHERE note_id = ?)",
                (note_id,)
            )
            self.conn.execute(
                "DELETE FROM relations WHERE tgt_chunk_id IN (SELECT id FROM chunks WHERE note_id = ?)",
                (note_id,)
            )
            # 删除任务
            ...
            print(f"[GraphStorage] delete_note error: {e}")
            return False
    ...
    def upsert_relation(self, relation: Relation) -> bool:
        ...
        try:
            ...
            print(f"[GraphStorage] upsert_relation error: {e}")
            return False
    ...
    def touch_relation(self, src_chunk_id: int, tgt_chunk_id: int, rel_type: str) -> bool:
        ...
        try:
            ...
            print(f"[GraphStorage] touch_relation error: {e}")
            return False

    def boost_relation(self, src_chunk_id: int, tgt_chunk_id: int, rel_type: str,
                       amount: float = 0.05, max_weight: float = 1.0) -> bool:
        """强化关系权重"""
        try:
            self.conn.execute(
                """UPDATE relations SET
            ...
            print(f"[GraphStorage] boost_relation error: {e}")
            return False
    ...
    def decay_old_relations(self, days: int, factor: float, min_weight: float = 0.1) -> int:
        ...
        try:
            ...
            print(f"[GraphStorage] decay_old_relations error: {e}")
            return 0

    # ==================== 图遍历 ====================

    def traverse_graph(self, seed_chunk_ids: list[int], max_hops: int = 2,
                       min_weight: float = 0.4, max_nodes: int = 50) -> list[dict]:
        """
        递归 CTE 图遍历

        返回: [(chunk_id, hop_distance, path_weight, path)]
        """
        ...
        query = f"""
            ...
            SELECT chunk_id, hop, path_weight, path
            FROM graph_traverse
        ...
        try:
            rows = self.conn.execute(
                query, (min_weight, max_hops, min_weight, max_nodes)
            ).fetchall()
            return [dict(r) for r in rows]
        except Exception as e:
            print(f"[GraphStorage] traverse_graph error: {e}")
            return []
    ...
    def set_representative_chunk(self, chunk_id: int, note_id: str) -> bool:
        ...
        try:
            self.conn.execute(
                "UPDATE chunks SET is_representative = 0 WHERE note_id = ?", (note_id,)
            )
            self.conn.execute(
                "UPDATE chunks SET is_representative = 1, note_id = ? WHERE id = ?",
                (note_id, chunk_id)
            )
            return True
        except Exception as e:
            print(f"[GraphStorage] set_representative_chunk error: {e}")
            return False
    ...
    def get_job_stats(self) -> dict:
        """获取任务统计"""
        rows = self.conn.execute(
            "SELECT status, COUNT(*) as cnt FROM graph_build_jobs GROUP BY status"
        ).fetchall()
    ...
    def record_tag_co_occurrence(self, tag1: str, tag2: str) -> bool:
        ...
        try:
            ...
            print(f"[GraphStorage] record_tag_co_occurrence error: {e}")
            return False
